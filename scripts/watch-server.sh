...
watch_config_arg="nodemon -e js,cjs,ts \
  ...
  -x \"node --experimental-specifier-resolution=node node_modules/webpack/bin/webpack.js -w --config webpack.server.$NODE_ENV.js\""
npx concurrently --names "SRV-WBPK,SRV" --prefix "{name}" --prefix-colors "blue" --kill-others \
  "$watch_config_arg" \
  "TZ=UTC nodemon --experimental-specifier-resolution=node --inspect --async-stack-traces --enable-source-maps --watch build/$NODE_ENV/server/ --delay 1 build/$NODE_ENV/server/main.js"
