...
watch_config_arg="nodemon -e js,cjs,ts \
  ...
  --watch app/server/types/generated/globals.d.ts \
...
npx concurrently --prefix " " --kill-others \
  "$watch_config_arg" \
  "TZ=UTC nodemon --es-module-specifier-resolution=node --inspect --async-stack-traces --enable-source-maps --watch build/$NODE_ENV/server/ --delay 2 build/$NODE_ENV/server/main.js"
