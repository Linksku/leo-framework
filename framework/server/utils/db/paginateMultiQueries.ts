...
export default async function paginateMultiQueries<T extends QueryBuilder<Model>>(
  queries: {
    query: T,
  ...
  if (queryIdx === null || !queries[queryIdx]) {
    throw new Error('paginateMultiQueries: invalid cursor.');
  }
  ...
  while (res.data.entityIds.length < limit / 2 && queryIdx < queries.length - 1) {
    if (!process.env.PRODUCTION && !res.data.hasCompleted) {
      throw new Error('paginateMultiQueries: query should be completed.');
    }
  ...
  if (!res.data.hasCompleted) {
    if (!process.env.PRODUCTION && !res.data.cursor) {
      throw new Error('paginateMultiQueries: result is missing cursor.');
    }
