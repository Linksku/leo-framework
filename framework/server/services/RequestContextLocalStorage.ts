...
function createCache() {
  ...
  return {
    ...
    delete(key: string) {
      if (process.env.NODE_ENV !== 'production' && (key in cache)) {
        size--;
      }
      delete cache[key];
    },
...
export function createRequestContext(req: ExpressRequest): RequestContext {
  ...
  const debug = process.env.NODE_ENV !== 'production' && !!params?.DEBUG;
  const profiling = process.env.NODE_ENV !== 'production' && !!params?.PROFILING;
