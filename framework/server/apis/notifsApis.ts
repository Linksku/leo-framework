...
import { NOTIF_SCOPES, NOTIF_SCOPES_ARR, UNSUB_NOTIF_ENTITIES } from 'config/notifs';
...
defineApi(
  ...
  async function notifsApi({
    ...
    const query = entityQuery(NotifModel)
    ...
    const generalNotifIds = notifs.filter(n => n.scope === NOTIF_SCOPES.GENERAL).map(n => n.id);
    ...
    const virtualRenderedNotifs = TS.filterNulls(renderedNotifs)
      .filter(notif => generalNotifIds.includes(notif.notifId))
