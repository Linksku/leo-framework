...
async function _fetchIncludedRelatedEntities(entities: Model[]): Promise<Model[]> {
  ...
  const flattened = results.flatMap(result => {
    if (Array.isArray(result.related)) {
      return result.related.concat(result.assocs);
    }
    if (result.related) {
  ...
  if (IS_PROFILING_APIS && flattened.length) {
...
export default async function formatApiSuccessResponse<Name extends ApiName>(
  ...
  if (Object.keys(others).length) {
    throw new UserFacingError(
      ...
      {
        status: 500,
        debugCtx: { additionalProperities: others },
      },
    );
  }
  ...
  if (entitiesFiltered.concat(createdEntitiesFiltered, updatedEntitiesFiltered)
  ...
  const allEntitiesDeduped = entitiesFiltered
    .concat(createdEntitiesFiltered, updatedEntitiesFiltered);
  ...
  if (!process.env.PRODUCTION) {
    ...
    // Lots of duplicates are expected for lists of similar items
    if (numSeen / related.length > 0.5 && numSeen >= 10) {
  ...
  const dateProps = _getDateProps(
    normalizedEntities
      .concat(normalizedCreatedEntities, normalizedUpdatedEntities),
  );
