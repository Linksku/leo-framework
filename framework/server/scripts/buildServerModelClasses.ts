...
async function getOutput(models: ModelsArr) {
  ...
  for (const { Model } of models) {
    ...
    declare static idColumn: ${
      Array.isArray(Model.idColumn)
        ? `['${Model.idColumn.join('\', \'')}']`
        : `'${Model.idColumn}'`
    };

    declare cls: ${Model.name}Class;
    declare relations?: ModelRelationsTypes<'${Model.type}'>;
...
export default async function buildServerModelClasses() {
