...
export default function ImageCropper({
  ...
  minDimPercent = 40,
  alt,
  // Makes scrolling not work with tall images
  // todo: low/easy enable moving cropped image
  enableMove = false,
  disabled,
}: {
  url: string,
  ...
  const bindCropped = useDrag(
    ({
      ...
      if (!img || !img.offsetWidth || !img.offsetHeight || canceled) {
        return;
      }
      if (first && disabled) {
        cancel();
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            right: `${right}%`,
            bottom: `${bot}%`,
            touchAction: enableMove && (top || left || right || bot)
              ? 'none'
              : undefined,
            boxShadow: disabled ? 'none' : undefined,
          }}
          {...(enableMove && (top || left || right || bot) ? bindCropped() : null)}
        >
          <img
            src={url}
            alt={alt}
